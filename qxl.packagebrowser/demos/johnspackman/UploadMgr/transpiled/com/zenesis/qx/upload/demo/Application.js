...
(function () {
  ...
  qx.Class.define("com.zenesis.qx.upload.demo.Application", {
    ...
    members: {
      ...
      main: function main() {
        // Call super class
        com.zenesis.qx.upload.demo.Application.prototype.main.base.call(this); // Enable logging in debug variant
        ...
        var uploadCount = 0; // Uploader controls the upload process; btn is the widget that will have the input[type=file]
        // attached, and "/demoupload" is the path files will be uploaded to (i.e. it's the value used
        // for the form's action attribute)
        //

        var uploadUrl = "http://www.zenesis.com/UploadMgr/demoupload"; //var uploadUrl = "http://localhost:9090/demoupload";

        var match = document.location.href.match(/uploadUrl=([^&]+)$/);
        if (match) uploadUrl = match[1];
        var uploader = new com.zenesis.qx.upload.UploadMgr(btn, uploadUrl); // Parameter tp be added to all uploads (can be overridden by
        // individual files)

        uploader.setParam("myGlobalParam", "global123"); // Optionally restrict the max number of simultaneous uploads
        // (default is 5)
        ...
        uploader.addListener("addFile", function (evt) {
          ...
          lst.add(item); // Set a parameter - each uploaded file has their own set, which
          // can override those set
          // globally against the upload manager

          ++uploadCount;
          file.setParam("myParam_" + uploadCount, "test");
          if (uploadCount % 2 == 0) file.setParam("myGlobalParam", "overridden-global-value"); // On modern browsers (ie not IE) we will get progress updates
          ...
          }, this); // All browsers can at least get changes in state (ie
...
//# sourceMappingURL=Application.js.map?dt=1613267505164