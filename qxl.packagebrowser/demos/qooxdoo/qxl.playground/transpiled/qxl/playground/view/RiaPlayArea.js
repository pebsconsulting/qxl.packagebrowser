  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.playground.view.RiaPlayArea", {
    ...
    construct: function construct() {
      ...
      this._caption.setLayout(new qx.ui.layout.HBox()); // caption label


      this.__P_431_0 = new qx.ui.basic.Label().set({
        font: "bold",
        padding: 7,
        allowGrowX: true,
        allowGrowY: true
      ...
      }, this); // combine all parts for the caption

      this._caption.add(this.__P_431_0);
      ...
      this.__P_431_1 = new qx.ui.container.Scroll();

      this.__P_431_1.getChildControl("scrollbar-x");

      this.__P_431_1.getChildControl("scrollbar-y");

      this.__P_431_1.getChildControl("corner");

      this._dummy = new qx.ui.core.Widget();

      this.__P_431_1.add(this._dummy);

      this.add(this.__P_431_1, {
        flex: 1
      });
    },
    events: {
    ...
    members: {
      ...
      init: function init() {
        ...
        this._playRoot.getLayoutParent = function () {
          return self.__P_431_1;
        };

        this.__P_431_1.getChildren = this.__P_431_1._getChildren = function () {
          return [self._playRoot];
        }; // copy the initial size which is availbale due to the flush at the beginning


        this._playRoot.setMinWidth(this.__P_431_1.getBounds().width);

        this._playRoot.setMinHeight(this.__P_431_1.getBounds().height);

        this.__P_431_1.addListener("resize", function (e) {
          var data = e.getData();
      ...
      reset: function reset(beforeReg, afterReg, code) {
        ...
        qx.ui.core.queue.Dispose.flush(); // clean up the registry. Only really new objects should be in
        ...
        for (var hash in afterReg) {
          if (!beforeReg[hash] && !afterReg[hash].isDisposed()) {
            ...
            if (code.indexOf(afterReg[hash].classname) != -1) {
              // if yes, dispose it
              var item = afterReg[hash]; // skip singletons
    ...
    destruct: function destruct() {
      this._disposeObjects("__P_431_0", "__P_431_1", "_dummy", "_playRoot", "_playApp");
    }
  });
  qxl.playground.view.RiaPlayArea.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=RiaPlayArea.js.map?dt=1613268347550