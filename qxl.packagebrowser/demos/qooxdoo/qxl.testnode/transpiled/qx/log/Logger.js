  ...
  qx.Bootstrap.define("qx.log.Logger", {
    statics: {
      ...
      */
      ...
      setTreshold: function (value) {
        this.__P_6_1.setMaxMessages(value);
      },
      ...
      getTreshold: function () {
        return this.__P_6_1.getMaxMessages();
      },
      ...
      */
      ...
      register: function (appender) {
        ...
        this.__P_6_2[id] = appender;
        this.__P_6_3[appenderName] = appender;
        appender.$$id = id; // Insert previous messages

        var entries = this.__P_6_1.getAllLogEvents();

        for (var i = 0, l = entries.length; i < l; i++) {
          var entry = entries[i];

          var appenders = this.__P_6_6(entry.loggerName, entry.level);
      ...
      unregister: function (appender) {
        ...
        var appenderName = appender.appenderName || appender.classname;
        delete this.__P_6_3[appenderName];
        delete this.__P_6_2[id];
        delete appender.$$id;
      },
      ...
      */
      ...
      debug: function (object, message) {
        qx.log.Logger.__P_6_7("debug", arguments);
      },
      ...
      info: function (object, message) {
        qx.log.Logger.__P_6_7("info", arguments);
      },
      ...
      warn: function (object, message) {
        qx.log.Logger.__P_6_7("warn", arguments);
      },
      ...
      error: function (object, message) {
        qx.log.Logger.__P_6_7("error", arguments);
      },
      ...
      trace: function (object, message) {
        if (qx.log.Logger.isLoggerEnabled("trace", object)) {
          ...
          qx.log.Logger.__P_6_7("trace", args);
        }
      },
      ...
      */
      ...
      __P_6_1: new qx.log.appender.RingBuffer(50),
      ...
      isLoggerEnabled: function (level, object) {
        var loggerName = this.__P_6_10(object);

        var appenders = this.__P_6_6(loggerName, level);
      ...
      __P_6_7: function (level, args) {
        ...
        for (var i = start, l = args.length; i < l; i++) {
          items.push(this.__P_6_11(args[i], true));
        } // Build entry
        ...
        this.__P_6_1.process(entry); // Send to appenders
       ...
       */
      __P_6_6: function (className, level) {
        var levels = this.__P_6_8; // If no filters, then all appenders apply

        if (!this.__P_6_4.length) {
          // Check the default level
          if (levels[level] < levels[this.__P_6_0]) {
            return [];
          }
        ...
        var appenders = this.__P_6_9[cacheId];
        ...
        for (var i = 0; i < this.__P_6_4.length; i++) {
          var filter = this.__P_6_4[i]; // Filters only apply to certain levels
          ...
          if (!filter.loggerMatch || filter.loggerMatch.test(className)) {
            if (filter.appenderName) {
              appenders[filter.appenderName] = this.__P_6_3[filter.appenderName];
            } else {
              return this.__P_6_9[cacheId] = this.__P_6_3;
            }
          }
        }

        return this.__P_6_9[cacheId] = appenders;
      },
       ...
       * <code>trace</code>.
       */
      __P_6_11: function (value, deep) {
        var type = this.__P_6_12(value);
          ...
          case "array":
            if (deep) {
              ...
              for (var i = 0, l = value.length; i < l; i++) {
                ...
                text.push(this.__P_6_11(value[i], false));
              }
            } else {
              text = "[...(" + value.length + ")]";
            }
          ...
          case "map":
            if (deep) {
              ...
              for (var i = 0, l = sorted.length; i < l; i++) {
                ...
                key = sorted[i];
                temp = this.__P_6_11(value[key], false);
                temp.key = key;
                text.push(temp);
              }
            } else {
    ...
    defer: function (statics) {
      ...
      for (var i = 0; i < logs.length; i++) {
        statics.__P_6_7(logs[i][0], logs[i][1]);
      }
...
//# sourceMappingURL=Logger.js.map?dt=1613268392746