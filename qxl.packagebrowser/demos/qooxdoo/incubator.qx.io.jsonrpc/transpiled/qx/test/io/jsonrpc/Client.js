  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.test.io.jsonrpc.Client", {
    ...
    members: {
      ...
      assertExceptionThrown: function assertExceptionThrown(response, exception) {
        ...
        const errorCallback = this.spy(err => {
          ...
          if (qx.lang.Type.isNumber(exception)) {
            ...
            this.assertInstance(err, exception, `Exception class does not match. Expected ${exception.classname}, got ${err}.`);
          }
        }); // check message promise

        message_out.getPromise().catch(errorCallback); // check event

        client.addListener("error", evt => errorCallback(evt.getData())); // check transport promise
      ...
      "test: call jsonrpc method and receive batched response": async function testCallJsonrpcMethodAndReceiveBatchedResponse() {
        this.resetId();
        let message_out = new qx.io.jsonrpc.protocol.Request("foo", ["bar"]);
        let result = "Hello World!";
        let response = new qx.io.jsonrpc.protocol.Batch().add(new qx.io.jsonrpc.protocol.Result(message_out.getId(), result)).addRequest("foo", ["bar"]).addNotification("logout").toString();
        this.setUpFakeServer(response);
        const client = new qx.io.jsonrpc.Client("http://jsonrpc");
        let spy = this.spy(value => this.assertEquals(result, value));
        message_out.getPromise().then(spy);
...
//# sourceMappingURL=Client.js.map?dt=1613267713386