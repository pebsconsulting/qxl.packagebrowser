  ...
  qx.Class.define("qxl.testtapper.Application", {
    ...
    members: {
      ...
      addTreeItem: function addTreeItem(status, testNumber, testClass, testName, message = "") {
        let classNode = this.__P_585_1.getChildren().toArray().find(item => item.getLabel() === testClass);
        ...
        if (!classNode) {
          ...
          this.__P_585_1.getChildren().append(classNode);
        }
        ...
        [classNode, this.__P_585_1].forEach(node => {
      ...
      main: function main() {
        ...
        const tree = this.__P_585_0 = new qx.ui.tree.VirtualTree(null, "label", "children");
        container.add(tree);
        const delegate = {
        ...
        let model = this.__P_585_1 = qx.data.marshal.Json.createModel(this.getRootNodeData(), true);
        ...
        return pChain.then(() => {
          ...
          this.__P_585_1.setLabel("Tests have finished:");
        });
      },
...
//# sourceMappingURL=Application.js.map?dt=1613267734160