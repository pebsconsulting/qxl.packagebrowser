(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.mobileshowcase.page.Maps", {
    ...
    members: {
      ...
      _initialize: function _initialize() {
        qxl.mobileshowcase.page.Maps.prototype._initialize.base.call(this);

        if (this._geolocationEnabled) {
          this._initGeoLocation();
        }

        this._loadMapLibrary(); // Listens on window orientation change and resize, and triggers redraw of map.
       ...
       * and drawing markers.
       */
      _redrawMap: function _redrawMap() {
        if (this._map !== null) {
      ...
      _createContent: function _createContent() {
        ...
        menuContainer.setId("mapMenu"); // LABEL

        var descriptionLabel = new qx.ui.mobile.basic.Label("Page Title");
        descriptionLabel.addCssClass("osmMapLabel"); // TOGGLE BUTTON

        var toggleNavigationButton = new qx.ui.mobile.form.ToggleButton(true, "ON", "OFF"); // SHOW MY POSITION BUTTON

        this._showMyPositionButton = new qx.ui.mobile.form.Button("Find me!");

        this._showMyPositionButton.addListener("tap", this._getGeoPosition, this); // Button is disabled when Geolocation is not available.
       ...
       */
      _loadMapLibrary: function _loadMapLibrary() {
       ...
       */
      _zoomToDefaultPosition: function _zoomToDefaultPosition() {
        if (this.isVisible()) {
       ...
       */
      _zoomToPosition: function _zoomToPosition(longitude, latitude, zoom, showMarker) {
        if (!this._map) {
          return;
        }
       ...
       * @param showMarker {Boolean} if a marker should be drawn at the defined position.
       */
      _setMarkerOnMap: function _setMarkerOnMap(map, mapPosition, showMarker) {
        if (this._marker) {
       ...
       * @param position
       */
      _onGeolocationSuccess: function _onGeolocationSuccess(position) {
       ...
       */
      _onGeolocationError: function _onGeolocationError() {
        ...
        qx.ui.mobile.dialog.Manager.getInstance().confirm(title, text, function () {}, this, buttons);
      },

      /**
       * Retreives GeoPosition out of qx.bom.Geolocation and zooms to this point on map.
       */
      _getGeoPosition: function _getGeoPosition() {
        var geo = qx.bom.GeoLocation.getInstance();
  ...
  qxl.mobileshowcase.page.Maps.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Maps.js.map?dt=1613268275587