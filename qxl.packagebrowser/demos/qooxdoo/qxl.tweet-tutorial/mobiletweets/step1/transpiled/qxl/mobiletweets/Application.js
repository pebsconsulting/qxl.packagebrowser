  ...
  qx.Class.define("qxl.mobiletweets.Application", {
    ...
    */
    members: {
      ...
      main: function main() {
        // Call super class
        qxl.mobiletweets.Application.prototype.main.base.call(this); // Enable logging in debug variant
        ...
        */
        ...
        var manager = new qx.ui.mobile.page.Manager(false); // Create an instance of the Input class and initial show it

        var inputPage = this.__P_584_0 = new qxl.mobiletweets.page.Input(); // Add page to manager

        manager.addDetail(inputPage); // Display inputPage on start

        inputPage.show(); // Create an instance of the Tweets class and establish data bindings

        var tweetsPage = new qxl.mobiletweets.page.Tweets();
        this.bind("tweets", tweetsPage, "tweets");
        this.bind("username", tweetsPage, "title"); // Add page to manager

        manager.addDetail(tweetsPage); // Create an instance of the Tweet class

        var tweetPage = new qxl.mobiletweets.page.TweetDetail(); // Add page to manager

        manager.addDetail(tweetPage); // Load the tweets and show the tweets page

        inputPage.addListener("requestTweet", function (evt) {
          this.setUsername(evt.getData());
          tweetsPage.show();
        }, this); // Show the selected tweet

        tweetsPage.addListener("showTweet", function (evt) {
          var index = evt.getData();
          tweetPage.setTweet(this.getTweets().getItem(index));
          tweetPage.show();
        }, this); // Return to the Input page

        tweetsPage.addListener("back", function (evt) {
          inputPage.show({
            reverse: true
          });
        }, this); // Return to the Tweets Page.
      ...
      __P_584_1: function __P_584_1() {
        ...
        store.setUrl(url); // Use data binding to bind the "model" property of the store to the "tweets" property
...
//# sourceMappingURL=Application.js.map?dt=1613269577878