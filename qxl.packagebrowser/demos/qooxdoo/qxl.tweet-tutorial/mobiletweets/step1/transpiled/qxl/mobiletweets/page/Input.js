  ...
  qx.Class.define("qxl.mobiletweets.page.Input", {
    ...
    members: {
      ...
      _initialize: function _initialize() {
        qxl.mobiletweets.page.Input.prototype._initialize.base.call(this);

        var title = new qx.ui.mobile.form.Title("Please enter an identi.ca username");
        this.getContent().add(title);
        var form = this.__P_586_0 = new qx.ui.mobile.form.Form();
        var input = this.__P_586_1 = new qx.ui.mobile.form.TextField();
        input.setPlaceholder("Username");
        input.setRequired(true);
        form.add(input, "Username"); // Add the form to the content of the page, using the SinglePlaceholder to render
        // the form.

        this.getContent().add(new qx.ui.mobile.form.renderer.Single(form)); // Create a new button instance and set the title of the button to "Show"

        var button = new qx.ui.mobile.form.Button("Show"); // Add the "tap" listener to the button

        button.addListener("tap", this._onTap, this); // Add the button the content of the page
      ...
      _onTap: function _onTap(evt) {
        // validate the form
        if (this.__P_586_0.validate()) {
          var username = this.__P_586_1.getValue();
...
//# sourceMappingURL=Input.js.map?dt=1613269577955