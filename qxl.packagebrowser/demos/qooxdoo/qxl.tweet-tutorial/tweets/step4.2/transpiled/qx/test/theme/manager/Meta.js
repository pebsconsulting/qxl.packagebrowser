(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.test.theme.manager.Meta", {
    ...
    members: {
      __P_269_0: null,
      __P_269_1: /(orange.*yellow|rgb\(255, 165, 0\).*rgb\(255, 255, 0\)|data:image\/png;base64,iVBORw0K)/,
      setUp: function setUp() {
        ...
        this.__P_269_0 = this.manager.getTheme(); // add a theme able widget

        this.__P_269_2 = new qx.ui.form.Button("Foo");
        this.getRoot().add(this.__P_269_2);
        qx.ui.core.queue.Manager.flush();
      },
      tearDown: function tearDown() {
        this.__P_269_2.destroy();

        this.manager.setTheme(this.__P_269_0);
        this.__P_269_0 = null;
      },
      testAllThemeManagerChanged: function testAllThemeManagerChanged() {
        ...
        var elem = this.__P_269_2.getContentElement().getDomElement(); // mocked appearance theme defines a padding with 30px 80px
        ...
        this.assertNotNull(qx.bom.element.Style.get(elem, "backgroundImage").match(this.__P_269_1)); // mocked decoration theme defines a border radius of 10 pixel

        this.assertEquals(qx.bom.element.Style.get(elem, "borderTopLeftRadius"), "10px"); // button label element

        elem = this.__P_269_2.getChildControl("label").getContentElement().getDomElement(); // mocked color theme defines red text color for button labels
      ...
      testColorThemeManagerChanged: function testColorThemeManagerChanged() {
        ...
        var elem = this.__P_269_2.getContentElement().getDomElement(); // mocked color theme defines a gradient with 'orange' and 'yellow';
        // also check for corresponding rgb values (need for FireFox)


        this.assertNotNull(qx.bom.element.Style.get(elem, "backgroundImage").match(this.__P_269_1));
      },
      testDecoratorThemeManagerChanged: function testDecoratorThemeManagerChanged() {
        ...
        var elem = this.__P_269_2.getContentElement().getDomElement();
...
//# sourceMappingURL=Meta.js.map?dt=1613269090930