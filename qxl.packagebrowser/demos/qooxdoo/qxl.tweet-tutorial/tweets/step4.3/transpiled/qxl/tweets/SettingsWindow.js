(function () {
  ...
  qx.Class.define("qxl.tweets.SettingsWindow", {
    ...
    construct: function construct() {
      ...
      var currentLocale = localeManager.getLocale(); // mark this for translation (should hold the langauge name)

      this.marktr("$$languagename"); // create a radio button for every available locale

      for (var i = 0; i < locales.length; i++) {
        var locale = locales[i];
        var languageName = localeManager.translate("$$languagename", [], locale);
        var localeButton = new qx.ui.form.RadioButton(languageName.toString()); // save the local as model

        localeButton.setModel(locale);
        radioGroup.add(localeButton); // preselect the current locale
...
//# sourceMappingURL=SettingsWindow.js.map?dt=1613269301364