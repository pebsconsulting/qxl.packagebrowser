  ...
  qx.Class.define("qx.bom.webfonts.Manager", {
    ...
    */
    construct: function construct() {
      ...
      this.__P_141_1 = {};
      this.__P_141_2 = [];
      this.__P_141_3 = this.getPreferredFormats();
    },
    ...
    */
    members: {
      ...
      */
      ...
      require: function require(familyName, sourcesList, callback, context) {
        ...
        if (qx.core.Environment.get("engine.name") == "mshtml" && (parseInt(qx.core.Environment.get("engine.version")) < 9 || qx.core.Environment.get("browser.documentmode") < 9)) {
          if (!this.__P_141_5) {
            this.__P_141_5 = new qx.event.Timer(100);

            this.__P_141_5.addListener("interval", this.__P_141_6, this);
          }

          if (!this.__P_141_5.isEnabled()) {
            this.__P_141_5.start();
          }

          this.__P_141_2.push([familyName, sources, fontWeight, fontStyle, comparisonString, version, callback, context]);
        } else {
          this.__P_141_7(familyName, sources, fontWeight, fontStyle, comparisonString, version, callback, context);
        }
      },
      ...
      remove: function remove(familyName, fontWeight, fontStyle) {
        var fontLookupKey = this.__P_141_8(familyName, fontWeight, fontStyle);

        var index = null;

        for (var i = 0, l = this.__P_141_0.length; i < l; i++) {
          if (this.__P_141_0[i] == fontLookupKey) {
            index = i;

            this.__P_141_9(familyName, fontWeight, fontStyle);
        ...
        if (index !== null) {
          qx.lang.Array.removeAt(this.__P_141_0, index);
        }

        if (fontLookupKey in this.__P_141_1) {
          this.__P_141_1[fontLookupKey].dispose();

          delete this.__P_141_1[fontLookupKey];
        }
      },
      ...
      removeStyleSheet: function removeStyleSheet() {
        ...
        if (this.__P_141_4) {
          qx.bom.Stylesheet.removeSheet(this.__P_141_4);
        }
      ...
      */
       ...
       */
      __P_141_8: function __P_141_8(familyName, fontWeight, fontStyle) {
       ...
       * @param context {Object?} Optional context for the callback function
       */
      __P_141_7: function __P_141_7(familyName, sources, fontWeight, fontStyle, comparisonString, version, callback, context) {
        var fontLookupKey = this.__P_141_8(familyName, fontWeight, fontStyle);

        if (!this.__P_141_0.includes(fontLookupKey)) {
          var sourcesMap = this.__P_141_10(sources);

          var rule = this.__P_141_11(familyName, fontWeight, fontStyle, sourcesMap, version);
          ...
          if (!this.__P_141_4) {
            this.__P_141_4 = qx.bom.Stylesheet.createElement();
          }

          try {
            this.__P_141_12(rule);
          } catch (ex) {}

          this.__P_141_0.push(fontLookupKey);
        }

        if (!this.__P_141_1[fontLookupKey]) {
          this.__P_141_1[fontLookupKey] = new qx.bom.webfonts.Validator(familyName, comparisonString, fontWeight, fontStyle);

          this.__P_141_1[fontLookupKey].setTimeout(qx.bom.webfonts.Manager.VALIDATION_TIMEOUT);

          this.__P_141_1[fontLookupKey].addListenerOnce("changeStatus", this.__P_141_13, this);
        }

        if (callback) {
          var cbContext = context || window;

          this.__P_141_1[fontLookupKey].addListenerOnce("changeStatus", callback, cbContext);
        }

        this.__P_141_1[fontLookupKey].validate();
      },

      /**
       * Processes the next item in the queue
       */
      __P_141_6: function __P_141_6() {
        if (this.__P_141_2.length == 0) {
          this.__P_141_5.stop();

          return;
        }

        var next = this.__P_141_2.shift();

        this.__P_141_7.apply(this, next);
      },
       ...
       */
      __P_141_13: function __P_141_13(ev) {
       ...
       */
      __P_141_10: function __P_141_10(sources) {
       ...
       */
      __P_141_11: function __P_141_11(familyName, fontWeight, fontStyle, sourcesMap, version) {
        var rules = [];
        var formatList = this.__P_141_3.length > 0 ? this.__P_141_3 : qx.bom.webfonts.Manager.FONT_FORMATS;

        for (var i = 0, l = formatList.length; i < l; i++) {
          var format = formatList[i];

          if (sourcesMap[format]) {
            rules.push(this.__P_141_14(format, sourcesMap[format], version));
          }
        }
       ...
       */
      __P_141_14: function __P_141_14(format, url, version) {
        if (version) {
          url += "?" + version;
        }
       ...
       */
      __P_141_12: function __P_141_12(rule) {
        var completeRule = "@font-face {" + rule + "}\n";

        if (qx.core.Environment.get("browser.name") == "ie" && qx.core.Environment.get("browser.documentmode") < 9) {
          var cssText = this.__P_141_15(this.__P_141_4.cssText);

          cssText += completeRule;
          this.__P_141_4.cssText = cssText;
        } else {
          this.__P_141_4.insertRule(completeRule, this.__P_141_4.cssRules.length);
        }
      },
       ...
       */
      __P_141_9: function __P_141_9(familyName, fontWeight, fontStyle) {
        ...
        for (var i = 0, l = document.styleSheets.length; i < l; i++) {
          ...
          if (sheet.cssText) {
            ...
            cssText = this.__P_141_15(cssText);
       ...
       */
      __P_141_15: function __P_141_15(cssText) {
    ...
    */
    destruct: function destruct() {
      if (this.__P_141_5) {
        ...
        this.__P_141_5.dispose();
      }

      delete this.__P_141_0;
      this.removeStyleSheet();

      for (var prop in this.__P_141_1) {
        this.__P_141_1[prop].dispose();
      }
...
//# sourceMappingURL=Manager.js.map?dt=1613268228355