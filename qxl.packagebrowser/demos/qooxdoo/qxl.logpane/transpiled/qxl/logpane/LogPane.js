  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.logpane.LogPane", {
    ...
    construct: function construct() {
      this.__P_3_0 = [["debug", "Debug", "icon/16/categories/system.png"], ["info", "Info", "icon/16/status/dialog-information.png"], ["warn", "Warning", "icon/16/status/dialog-warning.png"], ["error", "Error", "icon/16/status/dialog-error.png"]];
      ...
      this.__P_3_1 = new qx.ui.toolbar.ToolBar();

      this.__P_3_1.add(caption);

      this.__P_3_1.addSpacer();

      this.__P_3_1.setBackgroundColor("white");
      ...
      this.__P_3_1.add(clearButton);

      this.add(this.__P_3_1); // log pane
      ...
      this.__P_3_2 = new qx.log.appender.Element();
      qx.log.Logger.unregister(this.__P_3_2); // Directly create DOM element to use

      this.__P_3_3 = document.createElement("DIV");

      this.__P_3_2.setElement(this.__P_3_3);

      logArea.addListenerOnce("appear", function () {
        logArea.getContentElement().getDomElement().appendChild(this.__P_3_3);
      }, this);
    },
    properties: {
      /** Shows the toolbar */
    ...
    members: {
      ...
      fetch: function fetch(Logger) {
        ...
        Logger.register(this.__P_3_2); // Clear buffer
      ...
      _applyShowToolBar: function _applyShowToolBar(value, old) {
        if (value) {
          ...
          this.__P_3_1.exclude();
        }
      },
      // property apply
      _applyShowLogLevel: function _applyShowLogLevel(value, old) {
        if (!this.__P_3_4) {
          this.__P_3_4 = this.__P_3_5();

          this.__P_3_1.add(this.__P_3_4);
        }

        if (value) {
          this.__P_3_4.show();
        } else {
          this.__P_3_4.exclude();
        }
      },
      ...
      __P_3_5: function __P_3_5() {
        ...
        for (var i = 0, l = this.__P_3_0.length; i < l; i++) {
          var data = this.__P_3_0[i];
...
//# sourceMappingURL=LogPane.js.map?dt=1613268217138