(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.datademo.Demo", {
    ...
    members: {
      _createView: function _createView() {
        ...
        var controller = new qx.data.controller.List(null, list); // set the delegate

        controller.setDelegate(this); // set the name for the label property

        controller.setLabelPath("text"); // set the name for the icon property

        controller.setIconPath("user.profile_image_url"); // fetch some data from identi.ca

        var url = "qxl/datademo/service.js";
        var store = new qx.data.store.Jsonp();
        store.setCallbackName("callback");
        store.setUrl(url); // connect the store and the controller
        ...
        detailsBox.add(new qx.ui.basic.Label("Name: "), {
          row: 0,
          column: 0
        });
        ...
        detailsBox.add(new qx.ui.basic.Label("Message: "), {
          row: 2,
          column: 0
        });
        ...
        detailsBox.add(new qx.ui.basic.Label("Avatar: "), {
          row: 3,
          column: 0
        });
        var avatar = new qx.ui.basic.Image();
        detailsBox.add(avatar, {
          row: 3,
          column: 1
        });
        detailsController.addTarget(avatar, "source", "user.profile_image_url"); // connect the selected model item of the list to the detail view
  ...
  qxl.datademo.Demo.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Demo.js.map?dt=1613267988785